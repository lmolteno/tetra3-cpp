...
### Pi Zero
Higher-cost but opens up to the MIPI cameras. This seems required. The Pi Cam Module 3 NoIR works out of the box with 100ms exposures pretty reliably. Solve RMSE is about 12 arcseconds with the 72 degree FOV of the Module 3. This compares to astrometry.net `solve-field` which (by default, maybe there are parameters to tune) gives about a ~100 arcsecond RMSE. This was evaluated with a static camera watching the field drift and matching that to a sidereal model.
